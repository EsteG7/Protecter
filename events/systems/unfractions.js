...
module.exports = (client) => {
    try {
        cron.schedule("*/20 * * * *", async () => {
            ...
            console.log("[DATABASE] ".cyan + "Se ".green + `${unBanedUsers === 1 ? "ha" : "han"}`.green + " removido ".green + `${unBanedUsers}`.brightCyan + " " + `${unBanedUsers === 1 ? "baneo" : "baneos"}`.green + " y ".green + `${unMutedUsers} `.brightCyan + `${unMutedUsers === 1 ? "silencio" : "silencios"}`.green + ".".green);
        });
    } catch (error) {
